"""
Fruit Harvesting Model - Sandbox version (Weeks 1-2).
...
class HarvestModel(Model):
    ...
    def __init__(
        ...
        self.datacollector = DataCollector(
            model_reporters={
                'Total Yield': lambda m: sum(a.harvested for a in m.agents if isinstance(a, HarvesterAgent)),
                'Messages This Step': lambda m: sum(a.messages_sent for a in m.agents if isinstance(a, HarvesterAgent)),
                'Remaining Fruit': lambda m: sum(1 for f in m.fruits if f.available),
                'Available Fruit': lambda m: sum(1 for f in m.fruits if f.available),
            },
            agent_reporters={
                'Harvested': lambda a: a.harvested if isinstance(a, HarvesterAgent) else None,
                'Messages Sent': lambda a: a.messages_sent if isinstance(a, HarvesterAgent) else None,
            }
        )
